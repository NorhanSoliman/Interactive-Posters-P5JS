...
function displayRandomStrings(arr, positions) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (toggleMapping) {
      ...
      colorValue = map(mouseX, 0, width, 255, 50); // Light to dark
    } else {
      // Mapping 2: Dark to Light (start from dark, move to light as mouseX increases)
      colorValue = map(mouseX, 0, width, 50, 255); // Dark to light
    }

    fill(0,0,colorValue); // Apply the calculated color
